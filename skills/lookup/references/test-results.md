...
<!-- ABOUTME: All tests run 2026-03-29 against account ACxx...xx using Node.js SDK. -->

# Lookup v2 Test Results

All tests run 2026-03-29 against account ACxx...xx using Twilio Node.js SDK (`client.lookups.v2.phoneNumbers`).
...
|---|------|-------|--------|-------------|
| 1 | Basic lookup (no fields) | +15551234567 | `valid: true`, `validationErrors: []`, `countryCode: "US"` | `validationErrors` is empty array, not null |
...
| 3 | Line Type Intelligence | +15551234567 | `type: "nonFixedVoip"`, `carrierName: "Twilio - SMS/MMS-SVR"` | MCC/MNC populated for VoIP |
| 4 | Caller Name (CNAM) | +15551234567 | `callerName: null`, `callerType: "UNDETERMINED"` | "UNDETERMINED" is undocumented value |
| 5 | Multiple fields | +15551234567 | All three packages populated | Confirms multi-field in single request |
| 6 | SMS Pumping Risk | +15551234567 | `smsPumpingRiskScore: 34`, `carrierRiskCategory: "high"` | Score and carrier category are independent |
| 7 | SIM Swap | +15551234567 | `errorCode: 60606` (not enabled) | HTTP 200 with error in body |
| 8 | Line Status | +15551234567 | `errorCode: 60606` (not enabled) | Same pattern — error in body |
| 9 | Reassigned Number | +15551234567 | `errorCode: "60606"` (not enabled) | errorCode is STRING, not number |
| 10 | Call Forwarding | +15551234567 | `errorCode: 60607` | Different error code than 60606 |
| 11 | Quality Score | +15551234567 | `error_code: 60606`, snake_case keys | SDK uses snake_case (not camelCase) |
| 12 | Identity Match | +15551234567 | `errorCode: 60003` | VoIP number not eligible |
| 13 | Invalid field name | +15551234567 | HTTP 400 | Error message lists all valid field names |
| 14 | Non-E.164 with CountryCode | 5551234567, US | `phoneNumber: "+15551234567"`, `valid: true` | Correctly resolves national format |
| 15 | Validation field | +15551234567 | Same as no-fields | `validation` adds nothing extra |
| 16 | UK number line type | +442071234567 | `type: "nonFixedVoip"`, `carrierName: "Gamma Telecom..."` | International line type works |
| 17 | All available fields | +15551234567 | Full response with 4 packages, 7 null | Unrequested fields present as null; score changed to 41 |
...
| 20 | Reassigned without date | +15551234567 | `errorCode: "60617"` (string) | Missing LastVerifiedDate |
| 21 | SMS pumping UK | +442071234567 | `errorCode: 60006` | UK number returned error (not null) |
| 22 | TEST_PHONE_NUMBER | +15559876543 | Same as #3/#4 | Both Twilio numbers show nonFixedVoip |
...
Same number (+15551234567) returned `smsPumpingRiskScore: 34` in Test 6 and `smsPumpingRiskScore: 41` in Test 17, seconds apart. Scores are not deterministic — they reflect real-time model output.
