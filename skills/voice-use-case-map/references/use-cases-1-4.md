...
> **Migration note**: The legacy `IfMachine=Continue` parameter (synchronous AMD) creates ~4 seconds of dead air before connecting. Always use `MachineDetection=Enable` with `AsyncAmd=true` instead. If upgrading from older code that references `IfMachine`, `Twilio::REST::Client.new` (Ruby), or `new TwimlResponse()`, see the current Node.js SDK pattern: `client.calls.create({ machineDetection: 'Enable', asyncAmd: true, asyncAmdStatusCallback: '<url>' })`.
