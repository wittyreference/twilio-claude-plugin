---
name: "Video SDK"
description: "Comprehensive Twilio Video SDK patterns including room types, token generation, recordings, compositions, E2E testing, and key learnings from real implementations."
---
...
For automated validation, verify each checklist item via the Twilio Video REST API after room creation and participant connection.
...
A complete E2E test suite validates Video SDK integration using Playwright with real Twilio Video rooms.

### Running Video SDK Tests

Run the Playwright test suite in headless mode for CI, or headed mode for debugging. The tests exercise real Twilio Video rooms with Chrome's fake media devices.

### Test Harness

A complete E2E test harness consists of three components:

- A browser video app (HTML) with test-friendly DOM IDs for Playwright selectors
- An Express server serving the harness page and a token generation endpoint
- A Playwright test suite exercising all Video SDK features
...
The Playwright configuration for video tests uses:
...
The Twilio Video REST API provides tools for:
- **Room management**: Create, list, get, and update video rooms
- **Participants**: List, get, and update room participants
- **Recordings**: List room recordings, get/delete individual recordings
- **Compositions**: Create, list, get, and delete compositions and composition hooks
- **Tracks**: List subscribed and published tracks per participant
