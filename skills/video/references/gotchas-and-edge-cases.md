...
<!-- ABOUTME: Comprehensive gotchas and edge cases for Twilio Video development. -->
<!-- ABOUTME: Covers room lifecycle, composition timing, recording, reconnection, disconnect codes, and track limits. -->
...
**CRITICAL:** Compositions should only be created AFTER the room ends.

**Gotcha:** The API accepts composition requests on in-progress rooms (returns a CJ SID), but the composition ends with `failed` status because no completed recordings exist yet. This is NOT a hard API rejection — it's a silent failure. [Evidence: CJ9b42c9ccea2fd92f8baca76679c773ef returned 200 but status=failed]
...
Available regions: `us1`, `us2`, `ie1`, `de1`, `au1`, `br1`, `jp1`, `sg1`, `in1`, `gll` (global auto-select)

## Track Limits

**Gotcha:** Group rooms have track limits, but they are higher than commonly assumed.

Per Twilio docs (Programmable Video Limits), recommended per-room limits:
- 60 audio track publications
- 60 video track publications
- 50 data track publications

Per participant (subscriptions): 60 audio, 60 video, 49 data.
Per participant (publications): 2 audio, 6 video, 1 data.

`maxConcurrentPublishedTracks` is account-level (170 on standard accounts) and cannot be set per room — the API accepts but ignores the per-room value. [Evidence: all values 0/1/16/200 returned 170]
