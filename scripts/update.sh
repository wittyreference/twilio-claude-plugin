...
# ABOUTME: Detects remote setup, stashes local changes, merges upstream, and runs post-update steps.

set -euo pipefail

SCRIPT_DIR="$(cd "$(dirname "$0")" && pwd)"
PROJECT_ROOT="$(cd "$SCRIPT_DIR/.." && pwd)"
UPSTREAM_URL="https://github.com/wittyreference/twilio-claude-plugin.git"
...
# --- Post-update: install MCP deps if present ---
if [ -f "$PROJECT_ROOT/agents/mcp-servers/twilio/package.json" ]; then
    info "Rebuilding MCP server..."
    (cd "$PROJECT_ROOT/agents/mcp-servers/twilio" && npm install --quiet 2>/dev/null && npm run build --quiet 2>/dev/null)
    info "MCP server rebuilt."
fi

# --- Clear update cache ---
rm -rf "$PROJECT_ROOT/.update-cache"
