...
// ABOUTME: Prompts for credentials, provisions selected resources, deploys Functions, and updates .env. Uses project directory name for resource naming.
...
class TwilioSetup {
  constructor(accountSid, authToken) {
    ...
    this.projectName = path.basename(path.resolve(__dirname, '..'));
  }
  ...
  async createVerifyService() {
    ...
    try {
      const service = await this.client.verify.v2.services.create({
        friendlyName: `${this.projectName}-verify`,
        codeLength: 6,
      });
  ...
  async createSyncService() {
    ...
    try {
      const service = await this.client.sync.v1.services.create({
        friendlyName: `${this.projectName}-sync`,
      });
  ...
  async createMessagingService(phoneNumberSid = null) {
    ...
    try {
      const service = await this.client.messaging.v1.services.create({
        friendlyName: `${this.projectName}-messaging`,
  ...
  async createTaskRouterWorkspace() {
    ...
    try {
      const workspace = await this.client.taskrouter.v1.workspaces.create({
        friendlyName: `${this.projectName}-workspace`,
